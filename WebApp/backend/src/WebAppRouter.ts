...
passport.use(
  new LocalStrategy(
    ...
    async (email, password, done) => {
      ...
      try {
        const authenticateResult = await userModel.authenticateUser(
          email,
          password
        );
...
WebAppRouter.route('/logout').get((req, res) => {
  console.log('logout');
...
WebAppRouter.route('/signup').post(async (req, res) => {
...
WebAppRouter.route('/initialLoad').get(
  protectedRouteMiddleware,
  async (req: any, res) => {
    const machines = await machineModel.getMachinesForUser(req.user.companyId);
    const sales = await paymentModel.getSalesForMachines(
      machines.map(({ id }) => id)
    );
    const transactions = await paymentModel.getPaymentsForMachines(
      machines.map(({ id }) => id)
    );
...
WebAppRouter.route('/machine/restock').post(
  protectedRouteMiddleware,
  async (req: any, res) => {
...
WebAppRouter.route('/authenticated').get(
  protectedRouteMiddleware,
  (req: any, res) => {
    res.status(200).send();
  }
...
WebAppRouter.route('/machine/updateCapacity').post(
  protectedRouteMiddleware,
  async (req: any, res) => {
    const { machineId, companyId, items, strategy } = req.body;
    const machines = await machineModel.updateCapacity(
      machineId,
      companyId,
      strategy,
      items
