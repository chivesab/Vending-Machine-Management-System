    ...
    for (const strategy of SupplierTypes.STRATEGIES) {
      try {
        const response = await axios.get(
          `${ML_SERVER_URL}/${model}_${strategy}`
        );
        if (response.status !== 200) {
          throw new Error(response.status.toString());
        }

        for (const [machineName, items] of Object.entries<
        ...
        >(response.data)) {
          ...
          if (strategies[machineId] == null) {
            ...
            if (strategies[machineId][model] == null) {
              ...
              strategies[machineId][model][strategy as SupplierTypes.Strategy] =
                transformedItems as any;
            }
          }
        }
      } catch (e) {
        console.error(
          `Something went wrong when trying to get ML predictions for ${model}_${strategy}`
        );
        throw e;
      }
    }
