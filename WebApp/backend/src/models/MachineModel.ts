...
class MachineModel {
  async getMachinesForUser(companyId: string): Promise<SupplierTypes.Machine[]> {
  ...
  async updateCapacity(machineId: string, companyId: string, strategy: SupplierTypes.Strategy | undefined, items: SupplierTypes.Machine['items']): Promise<SupplierTypes.Machine[]>{
    const conn = await connection;
    const machine = await conn.query(`SELECT * from vending_machines WHERE id='${machineId}'`);
    if (machine.length === 0) {
  ...
  async getMachineForSupplier(machineId: string): Promise<SupplierTypes.Machine| undefined> {
    ...
    return {
      ...
      items: items.map(({ id, name, unit_price, quantity, capacity }: any) => ({
        itemId: id,
        name,
        price: unit_price,
        quantity,
...
const aggregateVMItems = (
  ...
  return ({
    ...
    ownerId: company_id,
    items: items.map(({ id, name, unit_price, quantity }) => ({
      id,
      name,
      price: unit_price,
      quantity
...
const aggregateMachines = (
  ...
  const transactionsByMachineId = _.groupBy(transactions, ({ machineId }) => machineId);

  return machines.map(({ id, name, floor, machine_number, company_id, city, longitude, latitude, strategy }: any): SupplierTypes.Machine => ({
    id,
    name,
    floor: floor,
    machineNumber: machine_number,
    companyId: company_id,
    items: _.sortBy(groupedItems[id]?.map((item) => ({
      id: item.id,
      name: item.name,
    ...
    sales: roundToTwoDecimals(_.sumBy(transactionsByMachineId[id] ?? [], ({totalPrice}) => totalPrice)),
    city,
    location: {
      longitude,
      latitude,
