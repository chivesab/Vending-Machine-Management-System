...
async function saveToDB(rows) {
  ...
  for (const value of Object.values(salesByMachines)) {
    ...
    for (const [dateString, sales] of Object.entries(salesByDate)) {
      ...
      const date = new Date(CURRENT_YEAR, Number(dateStrings[1]) - 1, Number(dateStrings[0]));
      ...
      for (const itemSale of sales) {
        ...
        if (!(itemKey in items)) {
          ...
          const size = itemSale.capacity === '1.5lt' ? 1500 :
            itemSale.capacity === '500ml' ? 500 : 330
          ...
          await conn.query('  \
            INSERT INTO items (id, name, itemKey, size, container) \
              VALUES (?, ?, ?, ?, ?)\
          ', [newItemId, ITEM_KEY_TO_NAME[itemKey], itemKey, size, itemSale.container.trim()]);
        ...
        if (!(itemId in machineItems[machineId])) {
          await conn.query( ' \
            INSERT INTO machine_items (machine_id, item_id, unit_price, quantity, capacity)\
              VALUES (?,?,?,?,?) \
            ', [machineId, itemId, unitPrice, getRandomInt(itemCapacity), itemCapacity]);
        ...
        for (let [d, numOfSales] of getDistribution(date, totalSale, getMachineDistribution(machineCounter))) {
          const payments = [];
          const paymentItems = [];
          let saleCounter = 0;
          while (saleCounter < numOfSales) {
            ...
            payments.push(`("${paymentId}", ${numOfItems * unitPrice}, "${getCardNumber()}", "${getMySQLDate(d)}", "${machineId}")`);
            paymentItems.push(`("${paymentId}","${itemId}",${unitPrice},${numOfItems})`);
...
function getMachineDistribution(counter) {
  return counter % 3 === 0 ? 'weekday' : counter % 3 === 1 ? 'weekend' : 'normal';
}
...
function getDistribution(date, quantity, type) {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    const weight = d.getDay() === 0 || d.getDay() === 6 ? SALES_DISTRIBUTION[type].weekend :
