...
export const Dashboard: React.FC = () => {
  ...
  const transactions = useSelector<any, SupplierTypes.Transaction[]>(
    ({ transactions }) => transactions
  ).sort((a, b) => {
    return new Date(a.timestamp) > new Date(b.timestamp) ? -1 : 1;
  });
  const transactionWidgetData = React.useMemo<any>(
    () => widgetData('transaction', transactions),
    [transactions]
  );
  const earningWidgetData = React.useMemo<any>(
    () => widgetData('earning', transactions),
    [transactions]
  );
    ...
    <div className="home">
      <div className="homeContainer">
        <div className="widgets">
          <Widget
            type={transactionWidgetData.type}
            amount={transactionWidgetData.amount}
            diff={transactionWidgetData.diff}
          />
          <Widget
            type={earningWidgetData.type}
            amount={roundToTwoDecimals(earningWidgetData.amount)}
            diff={earningWidgetData.diff}
          />
        </div>
  ...
  switch (type) {
    case 'transaction':
      while (
        ...
        new Date(transactions[i].timestamp) > lastWeek
      ) {
        amount++;
        i++;
      }
      while (
        i < transactions.length &&
        new Date(transactions[i].timestamp) > weekBeforeLastWeek
      ) {
        lastWeekAmount++;
        i++;
      }
