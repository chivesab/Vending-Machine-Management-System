...
export const MachineDetails: React.FC<Props> = ({ data: machine }) => {
  ...
  return (
    <div className="machine-details">
      ...
      <div className="machine-detail-stock-info"></div>
    </div>
  );
};
...
const filterMachineFromDaily = (
  daily: SupplierTypes.Sales['daily'][number],
  machineId: string
): SupplierTypes.Sales['daily'][number] => {
  const machineDaily = daily.machines.find(
    ({ machineId: mId }) => mId === machineId
  );
