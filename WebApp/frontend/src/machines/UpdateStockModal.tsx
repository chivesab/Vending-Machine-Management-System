...
export const UpdateStockModal: React.FC<Props> = ({machine,onClose, strategies}) => {
  ...
  const [strategy, setStrategy] = React.useState<SupplierTypes.Strategy | undefined>();
        ...
        { mode === 'auto' ? (
          <div className="ml-model-display">
            <Nav
              ...
              variant="tabs"
              defaultActiveKey="auto"
              onSelect={(selectedKey) => setModel(selectedKey as SupplierTypes.MLModel)}
            >
              <Nav.Item>
            ...
            {Object.entries(strategies[model]).map(([s, itemInfos]) =>{
              const onClick = () => {
                ...
                setItems(machine.items.map((item) => ({
                  ...item,
                  capacity: itemInfos[item.id]
                })));
              }

              const newSales = sum(machine.items.map(({id, price}) => price * itemInfos[id]));
              const diffPercentage = roundToTwoDecimals((newSales/currentSales-1)*100);
                ...
                <div className="strategy-option" key={s} onClick={onClick}>
                  <div className="strategy-option-text">
                    <label className="strategy-option-input">
                      <input type="radio" checked={s === strategy} onChange={() => undefined}/>
                      {s}
                    </label>
                    <div className={`stock-update-revenue-${diffPercentage >= 0 ? 'increase' : 'decrease'}`}>
                      {diffPercentage >= 0 ? '+' : ''}{diffPercentage} %
                    </div>
                  </div>
                  <StrategyDisplay
                    idToName={Object.fromEntries(machine.items.map(({id, name}) => [id, name]))}
                    originalCapacities={Object.fromEntries(machine.items.map(({id, capacity}) =>[id, capacity]) )}
...
const MachineItem: React.FC<ItemProps> = ({item, updatedItem, updateItem}) => {
  return (
    <tr className="machine-item-stock">
      ...
      <td>
        <div className="machine-item-stock-column">
          ...
          <Input type="number" value={updatedItem.price} onChange={(e) => {
      ...
      <td>
        <div className="machine-item-stock-column">
          ...
          <Input type="number" value={updatedItem.capacity} onChange={(e) => {
            const newCapacity = Number(e.target.value) || updatedItem.capacity;
            updateItem({
              ...updatedItem,
...
const StrategyDisplay: React.FC<StrategyDisplayProps> = ({
  idToName, originalCapacities, newCapacities
}) => {
  return (
    <div>
      <BarChart
