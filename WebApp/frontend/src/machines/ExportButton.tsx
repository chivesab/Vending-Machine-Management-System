...
export const ExportButton: React.FC<{
  ...
  <Button
    className="export-machines-button"
    variant="outlined"
    onClick={onExport}
  >
    Export
...
function machinesToTable(machines: SupplierTypes.Machine[]): TableMachine[] {
  const allItemNames = Array.from(
    new Set(_.flatMap(machines, ({ items }) => items.map(({ name }) => name)))
  );

  const machineTable = machines.map(
    ({ floor, machineNumber, sales, items }): TableMachine => ({
...
export function getStockLevel(items: SupplierTypes.Machine['items']): string {
  return (
    (_.sumBy(items, ({ quantity }) => quantity) /
      _.sumBy(items, ({ capacity }) => capacity)) *
      100 || 0
  ).toFixed(2);
}
