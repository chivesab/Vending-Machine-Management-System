...
const DEFAULT_VALUE: VendingMachineState = {
  ...
  list: [
    { id: '1', name: 'Mars', price: 3.5 },
    { id: '2', name: 'Snickers', price: 3.5 },
    { id: '3', name: 'Twix', price: 4.5 },
    { id: '4', name: 'Bounty', price: 3.5 },
    { id: '5', name: 'Tupla', price: 2.5 },
  ],
  basket: [
    { id: '1', name: 'Mars', price: 3.5, count: 1 },
    { id: '2', name: 'Snickers', price: 3.5, count: 2 },
    { id: '3', name: 'Twix', price: 4.5, count: 1 },
  ],
  modal: false,
  nearby: undefined,
};
...
export function reducer(
  state: VendingMachineState | undefined,
  action: ItemActions
) {
  ...
  switch (action.type) {
    case 'SELECT_ITEM':
      ...
      let basketIndex = newState.basket
        .map(function (e: any) {
          return e.id;
        })
        .indexOf(action.payload);
      let listIndex = newState.list
        .map(function (e: any) {
          return e.id;
        })
        .indexOf(action.payload);
    ...
    case 'REMOVE_ITEM':
      newState = cloneDeep(state);
      let index = newState.basket
        .map(function (e: any) {
          return e.id;
        })
        .indexOf(action.payload);
      const item = newState.basket.splice(index, 1)[0];
      const removedItemIndex = newState.list
        .map(function (e: any) {
          return e.id;
        })
        .indexOf(action.payload);
      newState.list[removedItemIndex].quantity =
        (newState.list[removedItemIndex].quantity || 0) + (item.count || 0);
    ...
    default:
      ...
      newState =
        history == null
          ? DEFAULT_VALUE
          : { ...DEFAULT_VALUE, ...JSON.parse(history) };
  }
